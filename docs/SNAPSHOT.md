...
| 3 | **Credenciales de producción de Flow** + `FLOW_API_BASE=https://www.flow.cl/api` en Vercel. **Antes de encender**: verificar `obtenerEstadoPagoFlow()` con un pago real chico — ese código nunca se probó contra un pago completado (TODO en `src/lib/flow.ts`), y es el mismo perfil del bug de firma que casi cuesta pagos con Khipu | Flow + Vercel | **Bloqueo B1**: hoy la tienda NO cobra plata real |
| 3b | **Configurar los medios de pago en el panel de Flow** (08-09-2026, decidido con las comisiones reales del panel): **activar** id 1 (tarjetas, 2,89%), id 5 (Onepay/billeteras), id 15 (MACH/BCI) e id 164 (cuotas sin tarjeta de crédito — el cliente paga en cuotas y el comercio no paga nada extra). **NO activar**: ids 130/131/132 (cuotas sin interés: +1,99% / +3,49% / **+6,99%** encima del 2,89% — a 12 cuotas la comisión real llega a **11,76% con IVA**, más de un tercio del margen de 30,9%), id 147 y 148 (3,19%), ni ids 22/17 (transferencia — ya está Khipu, más barato, y activarla rompería la regla de precios diferenciados) | Flow | Que la comisión no se coma el margen |
...
| 12 | **Cerrar las 4 decisiones del precio diferenciado** (`sevelin-tienda/docs/PLAN-PRECIOS-DIFERENCIADOS.md`, 08-09-2026): D1 ¿el recargo aplica al envío? · D2 ¿el POS presencial también cobra recargo con tarjeta, o queda incoherente con la web? · D3 ¿qué precio va al feed de Google/Meta? · D4 ¿todo el catálogo? | Decisión comercial | Desbloquea construir el precio doble (Fase 2). **Riesgo a vigilar: Google Merchant puede suspender productos si el precio del feed no calza con el del checkout** |
